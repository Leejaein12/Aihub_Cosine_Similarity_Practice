...
- 두 벡터의 내적을 각 벡터의 크기로 나눈 값
- `embed.py`에서 이미 L2 정규화를 해서 모든 벡터의 크기가 1
- 따라서: `cos(θ) = A·B / (1 × 1) = A·B`
- 그냥 내적만 하면 코사인 유사도가 나옴
- 결과 `sims`는 각 문장과 정답 문장의 유사도 점수 배열 `-1 ~ 1` 사이 값
...
**서버 시작**
1. 구글 드라이브에서 CSV + embeddings.npy 다운로드
2. 메모리에 로드

**사용자 요청**
1. 랜덤 문제 선택
2. 정답 벡터와 다른 문장들 코사인 유사도 계산
3. 적당히 비슷한 것 3개 오답으로 선택
4. 보기 4개 섞어서 반환
